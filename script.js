(function(){
  ...
  if(passReveal){
    const startPeek = ()=>{
      passInput.type = 'text';
      // mask most chars, only reveal last one as feedback
      try{ passInput.setSelectionRange(passInput.value.length, passInput.value.length); }catch(_){/* ignore */}
      // auto hide after 2s safety
      clearTimeout(passReveal._hideT);
      passReveal._hideT = setTimeout(endPeek, 2000);
    };
    const endPeek = ()=>{
      passInput.type = 'password';
      clearTimeout(passReveal._hideT);
    };
    passReveal.addEventListener('mousedown', startPeek);
    passReveal.addEventListener('touchstart', (e)=>{ e.preventDefault(); startPeek(); }, { passive:false });
  ...
  clickButton.addEventListener('touchstart', (ev)=>{
